...
基于原 `babymom-dashboard.html` 改造的可上线版本。每日**午盘收盘（11:35）+ 全日收盘（15:30）**两次自动计算「小白指数」（情绪化散户活跃度），推送到 GitHub Pages 展示；每次运行都会**留存快照**并支持**历史回溯**。
...
| **收盘权威主力** | **Tushare 免费档** | `stock_basic`(行业篮子) + `daily`(全市场批量,收盘后发布) + `index_daily`(上证) → 自聚合成板块，算出**全部小白指数维度** | ❌ 不需要（从海外/GitHub 直连） |
...
- 小白指数的 **6 个维度由 Tushare `daily` / 新浪实时 聚合算出**（单位已对齐：新浪成交额「元」→÷1000=千元、成交量「股」直接用；Tushare `vol`「手」→×100=股、`float_share`「股」）。
...
## 小白指数（情绪化散户活跃度）计算方法
对每个**行业板块**（由 Tushare `stock_basic` 的 `industry` 字段聚合，成分股 ≥ 5 只）按**过去 5 个交易日**聚合算 6 个周度维度 → **跨板块分位排名（0–100）** → 加权合成总分（降噪、看周度趋势）：

| 维度 | 含义 | 周度计算（Tushare `daily` + 新浪实时） |
|---|---|---|
| 拥挤度 | 周日均成交额相对更早历史日均的异动（钱往一处挤） | 近 5 日板块成交额均值 ÷ 更早历史日均（基线） |
| 扩散力 | 周内上涨家数占比（普涨/情绪一致） | 近 5 日上涨家数累计 ÷ 成分股数累计 |
| 动摇度 | 板块等权日收益本周年化波动率（上蹿下跳） | 本周日收益序列 std ×√252 |
| D回补 | 从本周低点回补的强度（一跌就有人抄） | 本周累乘日收益相对区间最低点涨幅 |
| 涨停密度 | 周内涨停个股占比（散户追板） | 近 5 日涨停家数累计 ÷ 成分股数累计 |
| 换手度 | 板块自由流通换手率（频繁倒手=散户特征） | 近 5 日 Σ(成交量→股) ÷ Σ(`float_share`)(股) |
...
**读分**：<40 冷静（机构主导）、40–70 温和、>70 过热（小白扎堆，回撤风险高）。
...
scripts/fetch_data.js 取数+计算脚本：新浪实时(盘中主源) + Tushare 免费档(收盘主力) + 东财(备份) → 小白指数 → data.json + 归档
